...
export default function BusinessHoursTab() {
  ...
  const DAYS = [
    t('settings.monShort'), t('settings.tueShort'), t('settings.wedShort'), t('settings.thuShort'),
    t('settings.friShort'), t('settings.satShort'), t('settings.sunShort'),
  ];
  ...
  const handleSave = async () => {
    setSaving(true);
    try { await apiFetch('/admin/business-hours', token, { method:'PUT', body:JSON.stringify(cfg) }); toast.success(t('settings.bhSaved')); }
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white">{t('settings.bhTitle')}</h3>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{t('settings.bhDesc')}</p>
      </div>
      ...
      <div className={card}>
        <label className="flex items-center gap-3 cursor-pointer">
          ...
          <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('settings.enableBh')}</span>
        </label>
        <p className="text-xs text-gray-400">{t('settings.bhDisabledHint')}</p>
      </div>

      <div className={card}>
        <h4 className="font-medium text-gray-800 dark:text-white">{t('settings.timezoneLabel')}</h4>
      ...
      <div className={card}>
        <h4 className="font-medium text-gray-800 dark:text-white">{t('settings.workingHours')}</h4>
        <div className="flex items-center gap-4 flex-wrap">
          <div>
            <label className="block text-xs text-gray-500 mb-1">{t('settings.startTime')}</label>
          ...
          <span className="text-gray-400 self-end pb-2">{t('settings.toLabel')}</span>
          <div>
            <label className="block text-xs text-gray-500 mb-1">{t('settings.endTime')}</label>
          ...
          <div className="text-xs text-gray-400 self-end pb-2">{t('settings.hoursPerDay').replace('{n}', cfg.end_hour - cfg.start_hour)}</div>
        </div>
      </div>

      <div className={card}>
        <h4 className="font-medium text-gray-800 dark:text-white">{t('settings.workingDays')}</h4>
        ...
        <p className="text-xs text-gray-400">
          {t('settings.workingDaysHint').replace('{days}', activeDays.length).replace('{hours}', activeDays.length * (cfg.end_hour - cfg.start_hour))}
        </p>
      </div>
      ...
      <button onClick={handleSave} disabled={saving} className="bg-indigo-600 text-white px-5 py-2 rounded-lg text-sm font-medium hover:bg-indigo-700 transition disabled:opacity-50">
        {saving ? t('settings.savingBh') : t('settings.saveBh')}
      </button>
    </div>
  );
}
